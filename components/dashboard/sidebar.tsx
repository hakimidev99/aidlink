...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { name: 'Wallet & Withdrawals', path: '/beneficiary/wallet', icon: WalletIcon },
    ...
    <>
      ...
      */}
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col justify-between border-r border-border-glass bg-white/80 p-6 backdrop-blur-3xl dark:bg-black/80 transition-transform duration-300 ease-in-out lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 lg:bg-white/40 lg:dark:bg-black/20 ${
        ...
        <div>
          ...
          <nav className="flex flex-col gap-2">
            {navItems.map((item) => {
              ...
              // Exact match for the root dashboard, partial match for sub-pages
              const isActive = item.path === '/beneficiary' 
                ? pathname === item.path 
        ...
        {/* Bottom Action / User Profile Snippet */}
        <div className="mt-auto">
          <div className="flex items-center gap-3 rounded-2xl border border-white/40 bg-white/30 p-4 backdrop-blur-md dark:border-white/10 dark:bg-black/30">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-primary to-secondary text-white shadow-inner">
              JD
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-bold text-text-heading dark:text-white">John Doe</span>
              <span className="text-xs text-text-muted">Beneficiary</span>
            </div>
          </div>
        </div>
      </aside>
